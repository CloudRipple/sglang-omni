...
def test_seedtts_benchmark_batch_args_default_to_64() -> None:
    ...
    assert results_config["request_timeout_s"] == 300


def test_seedtts_benchmark_request_timeout_is_configurable() -> None:
    config = _config_from_cli("--request-timeout-s", "1200")

    assert config.request_timeout_s == 1200
    results_config = _build_results_config(
        config,
        base_url="http://localhost:8000",
    )
    assert results_config["request_timeout_s"] == 1200
